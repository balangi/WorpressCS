public class RecentPostsWidget : Widget
{
    ...
    public override void Widget(Dictionary<string, object> args, Dictionary<string, object> instance)
    {
        var title = instance.GetValueOrDefault("title") as string ?? "Recent Posts";
    ...
    public override string Form(Dictionary<string, object> instance)
    {
        ...
        return $@"
            <p>
                <label>Title:</label>
                <input type='text' name='widget-{IdBase}[title]' value='{title}' />
            </p>";
    }
}